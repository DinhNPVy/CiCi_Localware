...
class Product extends MY_Controller
{
	...
	public function view()
	{
		...
		$brand_product = $this->brand_model->get_info($product->brand_id);
		$this->data['brand_product'] = $brand_product;
		...
		$input = array();
		$input['where'] = array('brand_id' => $product->brand_id);
		$input['limit'] = array('4', '0');
		$productsub_brand = $this->product_model->get_list($input);
		$this->data['productsub_brand'] = $productsub_brand;
	...
	public function brand()
	{
		...
		$this->data['brand_p'] = $brand;
		if (empty($catalog)) {
			redirect(base_url());
		}
		$input = array();
		if ($brand->parent_id == '1') {
			$input_brand = array();
			$input_brand['where'] = array('parent_id' => $brand->id);
			$input_brand['order'] = array('sort_order', 'ASC');
			$brand_child = $this->brand_model->get_list($input_brand);
			if (!empty($brand_child)) {
				$cat_list_id = array();
				foreach ($brand_child as $value) {
					$brand_list_id[] = $value->id;
				}
				$this->db->where_in('brand_id', $brand_list_id);
			} else {
		...
		$total = $this->brand_model->get_total($input);
		$this->data['total'] = $total;

		$this->load->library('pagination');
		$config = array();
		$base_url = base_url('product/brand/' . $id);
		$per = 8;
		$uri = 4;
		$config = pagination($base_url, $total, $per, $uri);
		$this->pagination->initialize($config);
		...
		$this->data['temp'] = 'site/product/brand';
		$this->load->view('site/layoutsub', $this->data);
	}
	public function hot()
	{
