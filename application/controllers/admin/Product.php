...
class Product extends MY_Controller
{
	...
	public function index()
	{
		...
		// $this->db->select('product.id as id,product.name as name,price,discount,image_link,view,buyed,catalog.name as namecatalog,brand.name as namebrand');
		// $this->db->join('catalog', 'catalog.id = product.catalog_id');
		// $product = $this->product_model->get_list($input);
		// $this->data['product'] = $product;
		$this->db->select('product.id as id,product.name as name,price,discount,image_link,view,buyed,catalog.name as namecatalog,brand.name as namebrand');

		$this->db->join('catalog',  'product.catalog_id = catalog_id');
		$this->db->join('brand', 'product.brand_id = brand_id');
	...
	public function add()
	{
		...
		$this->data['brand'] = $this->list_brand();
	...
	protected function list_brand()
	{
		...
		foreach ($brand as $value) {
			$input['where'] = array('parent_id' => $value->id);
			$subs = $this->brand_model->get_list($input);
			$value->sub = $subs;
		}
		return $brand;
	}
